<template>
                ...
                <form >
                    <div class="grid gap-4 mb-4 sm:grid-cols-1">
                        <div class="grid justify-between gap-4">
                            <div class="w-[750px] mt-3">
                                <label for="title" class="block mb-2 text-[16px] font-medium text-gray-900 dark:text-white">Bildirishnoma sarlavhasi</label>
                                <input v-model="contactInfo.title" type="text" name="title" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Sarlavha kiriting" required="">
                            </div>
                            <div class="w-[750px] mt-3">
                              <label for="body" class="block mb-2 text-[16px] font-medium text-gray-900 dark:text-white">Bildirishnoma haqida qisqacha</label>
                              <input v-model="contactInfo.body" type="text" name="body" id="body" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Bildirishnoma haqida qisqacha" required="">
                            </div>                                
                        </div>
                        <div class="">
    ...
    <!-- -------------------------end of modal------------------------------------- -->
        ...
        <a v-for="el in computedList" :key="el.id" href="#" class="flex flex-col items-center bg-[#F4F4F4] border border-gray-200 rounded-lg shadow md:flex-row lg:w-[45%] 2xl:w-[45%]  hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
            ...
            <div class="grid justify-between p-4 leading-normal">
                <h5 class="w-[250px] 2xl:w-[500px] mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">{{el.title}}</h5>
  ...
  const modifyContact=(event)=>{
      ...
      notification.createId(id, contact).then((res)=>{
          if(res.status == 200){
  ...
  const updateContact = (id)=>{
      ...
      contactInfo.body = foundContact[0].body
